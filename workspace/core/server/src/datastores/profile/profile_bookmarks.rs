...
#[derive(Clone)]
pub struct ProfileBookmarksDataStore {
    pool: TracingPool,
}

impl Debug for ProfileBookmarksDataStore {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProfileBookmarksDataStore").finish()
    }
}

impl ProfileBookmarksDataStore {
    ...
    pub async fn get_all(
        ...
        let connection = self.pool.get().await?;
        let stmt = connection
            .prepare_cached("select * from profile_bookmarks where profile_id = $1 order by created desc offset $2 limit $3")
            .await?;
