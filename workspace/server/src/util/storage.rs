...
pub async fn storage_system_metadata_delete(storage: &ObjectStorage, metadata: &Metadata, storage_systems: &[StorageSystem], client: &Client) -> Result<(), Error> {
...
pub async fn index_documents(ctx: &BoscaContext, documents: &[SearchDocumentInput], storage_system: &StorageSystem) -> Result<(), Error> {
...
pub async fn create_search_document(ctx: &BoscaContext, document: &SearchDocumentInput) -> Result<Option<Map<String, Value>>, Error> {
    Ok(if let Some(metadata_id) = &document.metadata_id {
        ...
        let metadata = ctx.check_metadata_action(&metadata_id_uuid, PermissionAction::Manage).await?;
        ...
        let collection = ctx.check_collection_action(&collection_id_uuid, PermissionAction::Manage).await?;
        let mut m = serde_json::Map::<String, Value>::new();
