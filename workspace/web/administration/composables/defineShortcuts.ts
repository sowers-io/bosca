...
export interface ShortcutsConfig {
  [key: string]: ShortcutConfig | (() => void)
}
  ...
  shortcuts = Object.entries(config).map(([key, shortcutConfig]) => {
    ...
    if (chained) {
      ...
      shortcut = {
        key: keySplit.filter((k) =>
          !['meta', 'ctrl', 'shift', 'alt'].includes(k)
        ).join('_'),
    ...
    if (!(shortcutConfig as ShortcutConfig).usingInput) {
      ...
      conditions.push(
        computed(() =>
          usingInput.value ===
            (shortcutConfig as ShortcutConfig).usingInput
        ),
      )
    }
