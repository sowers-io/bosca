...
import {
  type DocumentFragment, type DocumentTemplateFragment, type GuideFragment, type GuideTemplateFragment,
} from '~/lib/graphql/graphql.ts'

const breadcrumbs = useBreadcrumbs()
const route = useRoute()
const client = useBoscaClient()
const metadata = ref(await client.metadata.get(route.params.metadataId.toString()))
const relationships = ref(await client.metadata.getRelationships(route.params.metadataId.toString()))
const parents = ref(await client.metadata.getParents(route.params.metadataId.toString()))
...
if (metadata.value.content.type === 'bosca/v-document') {
  document.value = await client.metadata.getDocument(route.params.metadataId.toString())
  documentTemplate.value = document.value.template?.id && document.value.template?.version
  ...
  guide.value = await client.metadata.getGuide(route.params.metadataId.toString())
  guideTemplate.value = guide.value.template?.id && guide.value.template?.version
