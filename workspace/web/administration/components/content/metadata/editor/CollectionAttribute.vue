...
const props = defineProps<{
  ...
  onChange: (
      attribute: DocumentTemplateAttribute,
      collection: CollectionIdNameFragment[] | null,
  ) => void
...
const query = ref('')
const filter = ref('_type = "collection"' + (props.attribute?.configuration?.filter ? ' AND ' + props.attribute?.configuration?.filter : ''))
const offset = ref(0)
const limit = ref(50)
...
function onSelect(id: CollectionIdNameFragment) {
  ...
  props.onChange(props.attribute, newCollections)
  open.value = false
}

function onRemove(id: CollectionIdNameFragment) {
  let newCollections = collections.value.filter(c => c.id != id.id)
  collections.value = newCollections
  props.onChange(props.attribute, newCollections)
  open.value = false
}
...
<template>
  ...
  <div class="flex items-center justify-center">
    <TagsInput class="px-0 min-h-10 py-2 gap-0 w-full" v-model="collections">
      <div class="flex gap-2 flex-wrap items-center px-3">
        <TagsInputItem v-for="(item, index) in collections" :value="item.name">
          <TagsInputItemText/>
          <TagsInputItemDelete @click="onRemove(item)"/>
        </TagsInputItem>
      </div>

      <ComboboxRoot v-model:open="open" v-model:search-term="query" class="w-full" :filter-function="(val) => val">
        <ComboboxAnchor as-child>
          <ComboboxInput placeholder="Topics..." as-child>
            <TagsInputInput class="w-full px-3" :class="collections.length > 0 ? 'mt-2' : ''" @keydown.enter.prevent/>
          </ComboboxInput>
        </ComboboxAnchor>
