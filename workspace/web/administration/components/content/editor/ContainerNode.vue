...
import {AttributeUiType, type DocumentTemplateContainer} from "~/lib/graphql/graphql.js";
...
const nodeContainer = computed(() => {
  return props.extension.options.containers?.find(c => c.id === props.HTMLAttributes.name)
})
...
async function onRunWorkflow() {
  ...
  try {
    ...
    loading.value = false
    toast({title: 'Error Executing: ' + nodeContainer.value.description + ': ' + e.message})
  }
}
    ...
    if (key === nodeContainer.value?.supplementaryKey) {
      ...
      try {
        ...
        if (content?.content && content.content.length > 0) {
          const nodeContent = props.editor.schema.nodeFromJSON(content.content[0]);
  ...
  <NodeViewWrapper>
    <div class="container">
      <div class="container-name flex items-center justify-between">
        {{ nodeContainer?.name }}
        <template v-if="nodeContainer.workflows && nodeContainer.workflows.length > 0">
          <Button
