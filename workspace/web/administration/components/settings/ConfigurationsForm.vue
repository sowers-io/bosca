...
import type {ConfigurationFragment, PermissionInput} from '~/lib/graphql/graphql'
...
const configurations = (await bosca.configurations.getConfigurations()).map(c => reactive(new Configuration(c)))
...
async function saveConfigurations() {
  for (const config of configurations) {
    await bosca.configurations.setConfiguration({
      ...
      value: typeof config.value === 'string' ? JSON.parse(toRaw(config.value)) : toRaw(config.value),
    })
...
<template>
  <div class="space-y-4">
    ...
    <div v-for="(config, index) in configurations" :key="config.configuration.id" class="border rounded p-4">
      <div>
