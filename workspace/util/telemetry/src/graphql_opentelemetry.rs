// Original source: https://github.com/async-graphql/async-graphql/blob/master/src/extensions/opentelemetry.rs
//                  Copied due to compilation issues when importing opentelemetry.  When this is resolved,
//                  this should go away and be source from its original.
...
use opentelemetry::{trace::{FutureExt, SpanKind, TraceContextExt, Tracer}, Context as OpenTelemetryContext, Key, KeyValue};
use std::sync::Arc;
    ...
    <T as Tracer>::Span: Sync + Send,
{
        ...
        async move {
            ...
            if let Ok(doc) = &res {
                OpenTelemetryContext::current()
                    .span()
                    .set_attribute(KeyValue::new(KEY_SOURCE, ctx.stringify_execute_doc(doc, variables)));
            }
            res
        }
    ...
    async fn resolve(
        ...
        let fut = next.run(ctx, info).inspect_err(|err| {
            ...
            current_cx
                .span()
                .add_event("error".to_string(), vec![KeyValue::new(KEY_ERROR, err.to_string())]);
        });
