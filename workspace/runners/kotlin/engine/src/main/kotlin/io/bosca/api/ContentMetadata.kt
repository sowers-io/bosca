...
class ContentMetadata(network: NetworkClient) : Api(network) {
    ...
    suspend fun getBibleChapterContent(
        id: String,
        version: Int?,
        usfm: String
    ): BibleChapterContent? {
        val response = network.graphql.query(GetBibleMetadataChapterQuery(id, version.toOptional(), usfm)).execute()
        response.validate()
    ...
    suspend fun mergeAttributes(id: String, attributes: Any) {
        val response =
            network.graphql.mutation(MergeMetadataAttributesMutation(id, attributes))
                .execute()
        response.validate()
    }
